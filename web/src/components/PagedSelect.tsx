...
interface PagedSelectProps<TValue extends string | number, FetchParams extends PageableParams, TData> {
    value: TValue | null
...
export function PagedSelect<TValue extends string | number, FetchParams extends PageableParams, TData>({
                                                                                                           value,
                                                                                                           onChange,
                                                                                                           label,
                                                                                                           placeholder,
                                                                                                           ...
                                                                                                           initialValueLabel,
                                                                                                           additionalParams,
                                                                                                       }: PagedSelectProps<TValue, FetchParams, TData>) {
            ...
            <Popover open={isOpen} onOpenChange={setIsOpen}>
                <PopoverTrigger>
                    <Button
                        ...
                        role="combobox"
                        disabled={disabled}
                        className={cn("w-full justify-between", !value && !initialValueLabel && "text-muted-foreground", className)}
                        type="button"
                    >
                        {selectedLabel || initialValueLabel || placeholder || "Select an option"}
                        {
                            isOpen ?<ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50"/> :  <ChevronLeft className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                        }
                    </Button>
                </PopoverTrigger>
                <PopoverContent
